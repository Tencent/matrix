...
template<typename T, bool = std::is_abstract<T>::value> struct AlignmentCalcImpl {
    char x;
#if defined(_MSC_VER)
...
// of type T.
template<typename T> struct AlignmentCalcImpl<T, true>
: AlignmentCalcImplBase
, T {
...
template<typename T> struct AlignOf {
    ...
    static constexpr unsigned Alignment = static_cast<unsigned int>(sizeof(detail::AlignmentCalcImpl<T>) - sizeof(T));
#else
    enum { Alignment = static_cast<unsigned int>(sizeof(::llvm::detail::AlignmentCalcImpl<T>) - sizeof(T)) };
#endif
...
template<std::size_t Alignment, std::size_t Size> struct AlignedCharArray { alignas(Alignment) char buffer[Size]; };
...
#define LLVM_ALIGNEDCHARARRAY_TEMPLATE_ALIGNMENT(x) \
    template<std::size_t Size> struct AlignedCharArray<x, Size> { __attribute__((aligned(x))) char buffer[Size]; };
...
#define LLVM_ALIGNEDCHARARRAY_TEMPLATE_ALIGNMENT(x) \
    template<std::size_t Size> struct AlignedCharArray<x, Size> { __declspec(align(x)) char buffer[Size]; };
...
template<typename T1,
         ...
         typename T10 = char>
union SizerImpl {
    char arr1[sizeof(T1)], arr2[sizeof(T2)], arr3[sizeof(T3)], arr4[sizeof(T4)], arr5[sizeof(T5)], arr6[sizeof(T6)], arr7[sizeof(T7)],
...
: llvm::AlignedCharArray<AlignOf<llvm::detail::AlignerImpl<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>>::Alignment,
                         sizeof(::llvm::detail::SizerImpl<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>)> {};
} // end namespace llvm

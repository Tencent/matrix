...
- (BOOL)isMemoryCorruption:(NSDictionary *)report {
    ...
    for (NSDictionary *address in [notableAddresses objectEnumerator]) {
        ...
        if ([type isEqualToString:@"string"]) {
            ...
            if ([value rangeOfString:@"autorelease pool page"].location != NSNotFound && [value rangeOfString:@"corrupted"].location != NSNotFound) {
                return YES;
            }
...
- (NSString *)diagnoseCrash:(NSDictionary *)report {
    @try {
        ...
        if ([crashType isEqualToString:@KSCrashExcType_NSException]) {
            ...
            NSString *reason = [exception objectForKey:@KSCrashField_Reason] ? [exception objectForKey:@KSCrashField_Reason] :
                                                                               [errorReport objectForKey:@KSCrashField_Reason];
            return [self appendOriginatingCall:[NSString stringWithFormat:@"Application threw exception %@: %@", name, reason]
        ...
        if (zombieCall != nil) {
            return [self appendOriginatingCall:[NSString stringWithFormat:@"Possible zombie in call: %@", zombieCall] callName:lastFunctionName];
        }

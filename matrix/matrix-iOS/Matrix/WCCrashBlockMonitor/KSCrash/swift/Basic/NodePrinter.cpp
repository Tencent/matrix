...
#endif
            default: return "someKindaValueWitness";
        }
...
unsigned NodePrinter::printFunctionSigSpecializationParam(NodePointer Node,
    ...
    switch (K) {
        ...
        case FunctionSigSpecializationParamKind::ClosureProp:
            ...
            for (unsigned long e = Node->getNumChildren(); Idx < e;) {
...
void NodePrinter::printSpecializationPrefix(NodePointer node,
    ...
    for (unsigned long i = 0, e = node->getNumChildren(); i < e; ++i) {
...
#include "ReferenceStorage.def"
        ...
        case Node::Kind::GenericSpecializationParam:
            print(Node->getChild(0));
            for (unsigned long i = 1, e = Node->getNumChildren(); i < e; ++i) {
                if (i == 1)
                    Printer << " with ";
                else
                    Printer << " and ";
...
//            LLVM_FALLTHROUGH;
        ...
        case Node::Kind::DependentGenericSignature: {
            ...
            unsigned depth = 0;
            unsigned long numChildren = Node->getNumChildren();
            for (;
                 depth < numChildren
        ...
        case Node::Kind::SILBoxTypeWithLayout: {
            ...
            if (genericArgs) {
                Printer << " <";
                for (unsigned long i = 0, e = genericArgs->getNumChildren(); i < e; ++i) {
                    if (i > 0)
                        Printer << ", ";
...
printEntity(NodePointer Entity, bool asPrefixContext, TypePrinting TypePr,
    ...
    bool MultiWordName = ExtraName.count(' ') > 0;

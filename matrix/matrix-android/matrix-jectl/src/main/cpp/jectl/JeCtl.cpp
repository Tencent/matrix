...
Java_com_tencent_matrix_jectl_JeCtl_initNative(JNIEnv *env, jclass clazz) {
#ifdef __LP64__
    return ;
#else
    if (!initialized) {
...
Java_com_tencent_matrix_jectl_JeCtl_compactNative(JNIEnv *env, jclass clazz) {
...
static void *sub_routine(void *arg) {
    LOGD(TAG, "arg = %zu ", *((size_t *) arg));
    ...
    if (which_arena == 0) {
        call_alloc_large_in_arena1(*(size_t *) arg);
        free(p);
        free(arg);
        flush_decay_purge();
        return nullptr;
    ...
    pthread_create(&next_thread, nullptr, sub_routine, arg);
...
static int hack_prealloc_within_arena1(size_t size) {
    ...
    LOGD(TAG, "size1 = %zu", size);
    auto p_size = (size_t *) malloc(sizeof(size_t));
    *p_size = size;

    pthread_t next_thread;
    pthread_create(&next_thread, nullptr, sub_routine, p_size);
...
JNIEXPORT jint JNICALL
Java_com_tencent_matrix_jectl_JeCtl_preAllocRetainNative(JNIEnv *env, jclass clazz, jint __size0,
...
Java_com_tencent_matrix_jectl_JeCtl_getVersionNative(JNIEnv *env, jclass clazz) {
#ifdef __LP64__
...
Java_com_tencent_matrix_jectl_JeCtl_setRetain(JNIEnv *env, jclass clazz, jboolean enable) {
#ifdef __LP64__
    return true;
#else
    bool old = true;

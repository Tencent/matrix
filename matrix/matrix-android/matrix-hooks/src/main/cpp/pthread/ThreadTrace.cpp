...
//    std::regex regex(__regex_str);
    ...
    static void *wrapped_pthread_routine(void *arg) {
    ...
    thread_trace::routine_wrapper_t *wrap_pthread_routine(pthread_routine_t start_routine, void *args) {
        auto *routine_wrapper = (thread_trace::routine_wrapper_t *) malloc(sizeof(thread_trace::routine_wrapper_t));
        routine_wrapper->wrapped_func = wrapped_pthread_routine;
    ...
    // notice: 在父线程回调此函数
    void on_pthread_create(pthread_t *__pthread_ptr, pthread_attr_t const *__attr,
            pthread_routine_t __start_routine, void *__args) {
        ...
        pid_t tid = pthread_gettid_np(*__pthread_ptr);

        if (!rp_acquire()) {
            LOGD(TAG, "reentrant!!!");
            notify_routine(*__pthread_ptr);
            return;
        }
        //
        ...
        bool recorded = on_pthread_create_locked(*__pthread_ptr, java_stacktrace, tid);
     ...
     * @param __name
     */
    void on_pthread_setname_np(pthread_t __pthread, const char *__name) {
        if (NULL == __name) {
    ...
    void on_pthread_destroy(void *specific) {

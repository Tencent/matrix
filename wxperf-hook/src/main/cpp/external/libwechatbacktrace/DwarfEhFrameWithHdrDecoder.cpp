...
template <typename AddressType>
bool DwarfEhFrameWithHdrDecoder<AddressType>::Init(uint64_t offset, uint64_t, int64_t section_bias) {
...
template <typename AddressType>
bool DwarfEhFrameWithHdrDecoder<AddressType>::GetFdeOffsetFromPc(uint64_t pc, uint64_t* fde_offset) {
  if (fde_count_ == 0) {
    return false;
  }

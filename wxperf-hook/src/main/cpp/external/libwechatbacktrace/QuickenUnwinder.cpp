...
QutErrorCode WeChatQuickenUnwind(ArchEnum arch, uptr* regs, uptr* backtrace, uptr frame_max_size, uptr &frame_size) {
    ...
    for (; frame_size < frame_max_size;) {
        ...
        if (last_map_info && last_map_info->start <= cur_pc && last_map_info->end > cur_pc) {
            ...
            if (!interface) {
                QUT_DEBUG_LOG(WECHAT_QUICKEN_UNWIND_TAG, "Quicken interface is null, maybe the elf is invalid.");
                backtrace[frame_size] = PC(regs) - 2;
        ...
        QUT_TMP_LOG("WeChatQuickenUnwind pc_adjustment:%llx, step_pc:%llx, rel_pc:%llx", pc_adjustment, step_pc, rel_pc);
        ...
        QUT_TMP_LOG("WeChatQuickenUnwind Stepped Reg. r4: %x, r7: %x, r10: %x, r11: %x, sp: %x, ls: %x, pc: %x",
...
static inline void FpUnwind(uptr pc, uptr fp, uptr stack_top, uptr stack_bottom,
                                uptr * backtrace, uptr * frame_pointer, uptr frame_max_size, uptr &frame_size) {
...
QutErrorCode WeChatQuickenUnwindV2_WIP(ArchEnum arch, uptr* regs, uptr* backtrace, uptr frame_max_size, uptr &frame_size) {
    ...
    QUT_TMP_LOG("WeChatQuickenSmartUnwind start pc:%llx, fp:%llx", (uint64_t)pc, (uint64_t)fp);

    FpUnwind(pc, fp, stack_top, stack_bottom, backtrace, frame_pointer, frame_max_size, frame_size);
...
//        QUT_DEBUG_LOG("WeChatQuickenUnwind pc_adjustment:%llx, step_pc:%llx, rel_pc:%llx", pc_adjustment, step_pc, rel_pc);
        bool found = false;
        if (!interface->StepBack(step_pc, cur_sp, cur_fp, regs_bits, regs, process_memory_.get(), &found)) {
            ret = interface->last_error_code_;
            QUT_TMP_LOG("WeChatQuickenSmartUnwind failed. r4: %x, r7: %x, r10: %x, r11: %x, sp: %x, ls: %x, pc: %x",
                        R4(regs), R7(regs), R10(regs), R11(regs),SP(regs), LR(regs), PC(regs)
            );
            break;
        }
    ...
    QUT_TMP_LOG("WeChatQuickenSmartUnwind Reg restore end. r4: %x, r7: %x, r10: %x, r11: %x, sp: %x, ls: %x, pc: %x",
                R4(regs), R7(regs), R10(regs), R11(regs),SP(regs), LR(regs), PC(regs)
    );

...
if (entry_pair->entry_point == 0x13a0b8) {
        ...
        // re-encode it.
        if (QuickenInstructionsEncode(*it->second.second.get(), entry_pair->encoded_instructions, &prologue_conformed)) {
            // Well done.
...
if (entry_pair->entry_point == 0x13a0b8) {
    ...
    LOGE("WTF--Carl", "PackEntriesToFutSections bad: %llu, prologue: %llu, total: %llu", (uint64_t)bad_entries_count, (uint64_t)prologue_count, (uint64_t)entries_encoded.size());

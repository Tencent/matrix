...
//e54f8
if (entry_pair->entry_point == 0xe54f8) {
        QUT_DEBUG_LOG("PackEntriesToFutSections entry_pair->entry_point %x", entry_pair->entry_point);
}
        ...
        if (QuickenInstructionsEncode(*it->second.second.get(), entry_pair->encoded_instructions)) {
            ...
            bad_entries_count++;
            // Error occurred if we reached here. Add QUT_FINISH for this entry point.
            entry_pair->encoded_instructions.push_back(QUT_FINISH_OP);
        }
        ...
        if (it->second.second->size() > 3) {
            instr_tbl_count += ((it->second.second->size() + 3) / 4);
        }
        it++;
    }
        ...
        if (size_of_deque <= 3) {
            ...
            for (size_t i = 0; i < 3; i++) {
                ...
                if (size_of_deque > i) {
                    ...
                    compact |= (QUT_FINISH_OP << left_shift);
                }
            }
...
//            QUT_DEBUG_LOG("PackEntriesToFutSections size_of_deque %u size_ceil %u", size_of_deque, size_ceil);
            for (size_t i = 0; i < size_ceil; i++) {
                ...
                if (size_of_deque > i) {
                    ...
                    // Padding QUT_FINISH behind.
                    row |= (QUT_FINISH_OP << left_shift);
                }
...
//            QUT_DEBUG_LOG("PackEntriesToFutSections row_count %u, last_tbl_size %u", row_count, last_tbl_size);
    ...
    QUT_DEBUG_LOG("QuickenInterface::PackEntriesToFutSections idx_size %u, tbl_size %u", idx_size, tbl_size);
...
bool QuickenTableGenerator<AddressType>::GenerateUltraQUTSections(
    ...
    QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections debug_frame_instructions %u", debug_frame_instructions->size());
    ...
    QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections eh_frame_instructions %u", eh_frame_instructions->size());
    ...
    QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections gnu_debug_frame_instructions %u", gnu_debug_frame_instructions->size());
    ...
    QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections gnu_eh_frame_instructions %u", gnu_eh_frame_instructions->size());
    ...
    if (arm_exidx_info.size_ != 0) {
        ...
        QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections exidx_instructions %u", exidx_instructions->size());
        merged = MergeFrameEntries(merged, exidx_instructions);
    }

    QUT_DEBUG_LOG("QuickenInterface::GenerateUltraQUTSections merged %u", merged->size());
...
inline bool QuickenTable::Decode(uint32_t* instructions, const size_t amount, size_t start_pos) {
    ...
    QUT_DEBUG_LOG("QuickenTable::Decode instructions %x, amount %u, start_pos %u", instructions, amount, start_pos);
...
bool QuickenTable::Eval(uint32_t entry_offset) {
    ...
    QUT_DEBUG_LOG("QuickenTable::Eval entry_offset %u, add %x, command %x", entry_offset, fut_sections_->quidx[entry_offset], fut_sections_->quidx[entry_offset + 1]);
    if (command >> 31) {
        return Decode(&command, 1, 2); // compact
    } else {
        uint32_t row_count = command >> 24 & 0x7f;
        uint32_t row_offset = command & 0xffffff;
        QUT_DEBUG_LOG("QuickenTable::Eval row_count %u, row_offset %u", row_count, row_offset);
        return Decode(&fut_sections_->qutbl[row_offset], row_count, 3);
    }
}

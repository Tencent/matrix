        ...
        switch (operand) {
            ...
            case DwarfCfaInfo::DWARF_DISPLAY_ADDRESS:
                if (std::is_same<AddressType, uint32_t>::value) {
                    string += android::base::StringPrintf(" 0x%" PRIx32,
                                                          static_cast<uint32_t>(value));
                } else {
                    string += android::base::StringPrintf(" 0x%" PRIx64,
...
// Static data.
    ...
    bool DwarfCfa<AddressType>::cfa_set_loc(dwarf_loc_regs_t *) {
        ...
        if (new_pc < cur_pc) {
            if (std::is_same<AddressType, uint32_t>::value) {
                log(0, "Warning: PC is moving backwards: old 0x%" PRIx32 " new 0x%" PRIx32, cur_pc,
                    new_pc);
            } else {
                log(0, "Warning: PC is moving backwards: old 0x%" PRIx64 " new 0x%" PRIx64, cur_pc,
                    new_pc);
            }
        }
        cur_pc_ = new_pc;

        DWARF_CFA_LOG("DwarfCfa:   cfa_set_loc 0x%"
                              PRIxPTR
                              " ;", new_pc);
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_advance_loc(dwarf_loc_regs_t *) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_advance_loc cur_pc_ += 0x%x ;",
                      (uint32_t) (operands_[0] * fde_->cie->code_alignment_factor));
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_offset(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_offset reg(%u) = offset(0x%x) ;", (uint32_t) operands_[0],
                      (uint32_t) operands_[1]);
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_register(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_register reg(%u) = reg(%u) ;", (uint32_t) operands_[0],
                      (uint32_t) operands_[1]);
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa(dwarf_loc_regs_t *loc_regs) {
        (*loc_regs)[CFA_REG] = {.type = DWARF_LOCATION_REGISTER, .values = {operands_[0],
                                                                            operands_[1]}};

        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa CFA_REG = (%x, %x) ;", (uint32_t) operands_[0],
                      (uint32_t) operands_[1]);
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa_register(dwarf_loc_regs_t *loc_regs) {
        ...
        if (cfa_location == loc_regs->end() ||
            cfa_location->second.type != DWARF_LOCATION_REGISTER) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa_register CFA_REG = reg(%u) ;",
                      (uint32_t) operands_[0]);
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa_offset(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa_offset CFA_REG = offset(%u) ;",
                      (uint32_t) operands_[0]);
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa_expression(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa_expression CFA_REG = (%x, %x) ;",
                      (uint32_t) operands_[0], (uint32_t) memory_->cur_offset());
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_expression(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_expression reg(%u) = (%x, %x) ;", (uint32_t) operands_[0],
                      (uint32_t) operands_[1], (uint32_t) memory_->cur_offset());
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_offset_extended_sf(dwarf_loc_regs_t *loc_regs) {
        ...
        (*loc_regs)[reg] = {.type = DWARF_LOCATION_OFFSET, .values = {
                static_cast<uint64_t>(value)}};

        DWARF_CFA_LOG("DwarfCfa:   cfa_offset_extended_sf reg(%u) = offset(%x) ;",
                      (uint32_t) operands_[0], static_cast<uint32_t>(value));
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa_sf(dwarf_loc_regs_t *loc_regs) {
        SignedType offset =
                static_cast<SignedType>(operands_[1]) * fde_->cie->data_alignment_factor;
        (*loc_regs)[CFA_REG] = {.type = DWARF_LOCATION_REGISTER,
                .values = {operands_[0], static_cast<uint64_t>(offset)}};

        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa_sf CFA_REG = reg(%u) + %x ;",
                      (uint32_t) operands_[0], static_cast<uint32_t>(offset));
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_def_cfa_offset_sf(dwarf_loc_regs_t *loc_regs) {
        ...
        SignedType offset =
                static_cast<SignedType>(operands_[0]) * fde_->cie->data_alignment_factor;
        cfa_location->second.values[1] = static_cast<uint64_t>(offset);

        DWARF_CFA_LOG("DwarfCfa:   cfa_def_cfa_offset_sf CFA_REG = %x ;",
    ...
    bool DwarfCfa<AddressType>::cfa_val_offset(dwarf_loc_regs_t *loc_regs) {
        ...
        SignedType offset =
                static_cast<SignedType>(operands_[1]) * fde_->cie->data_alignment_factor;
        (*loc_regs)[reg] = {.type = DWARF_LOCATION_VAL_OFFSET, .values = {
                static_cast<uint64_t>(offset)}};

        DWARF_CFA_LOG("DwarfCfa:   cfa_val_offset reg(%u) = %x ;", (uint32_t) operands_[0],
                      static_cast<uint32_t>(offset));
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_val_offset_sf(dwarf_loc_regs_t *loc_regs) {
        ...
        (*loc_regs)[reg] = {.type = DWARF_LOCATION_VAL_OFFSET, .values = {
                static_cast<uint64_t>(offset)}};
        DWARF_CFA_LOG("DwarfCfa:   cfa_val_offset_sf reg(%u) = %x ;", (uint32_t) operands_[0],
                      static_cast<uint32_t>(offset));
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_val_expression(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_val_expression reg(%u) = (%x, %x) ;",
                      (uint32_t) operands_[0], (uint32_t) operands_[1],
                      (uint32_t) memory_->cur_offset());
        return true;
    }
    ...
    bool DwarfCfa<AddressType>::cfa_gnu_negative_offset_extended(dwarf_loc_regs_t *loc_regs) {
        ...
        DWARF_CFA_LOG("DwarfCfa:   cfa_gnu_negative_offset_extended reg(%u) = offset(%x) ;",
                      (uint32_t) operands_[0], static_cast<uint32_t>(offset));
        return true;
    }
